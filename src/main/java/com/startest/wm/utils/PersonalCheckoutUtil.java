 ...
 * @描述 任务信息导出帮助
 */
public class PersonalCheckoutUtil{
    /**
     * @Description: 导出任务监控列表
     * @Param: [dataList, httpServletResponse]
     * @return: void
     **/
    public static void exportCheckListtoExcel(List<wm_quality_user> dataList, HttpServletResponse httpServletResponse) {
        String[] columnHeaders = {"序号", "姓名", "图号", "类型", "工天", "平均工天", "工天因数", "消灭错漏率", "标准成绩","质量评定"};
        ...
        try {
            ...
            for (int i = 0; i < dataList.size(); i++) {
                wm_quality_user data = dataList.get(i);
                HSSFRow row = sheet.createRow(i + 1);
                HSSFCell cell0 = row.createCell(0);//序号
                cell0.setCellValue(data.getQuser_id());
                HSSFCell cell1 = row.createCell(1);
                cell1.setCellValue(data.getUser_name());//姓名
                HSSFCell cell2 = row.createCell(2);
                cell2.setCellValue(data.getProduct_code());//图号
                HSSFCell cell3 = row.createCell(3);
                cell3.setCellValue(data.getProduct_type());//类型
                HSSFCell cell4 = row.createCell(4);
                cell4.setCellValue(data.getUser_workdays());//工天
                HSSFCell cell5 = row.createCell(5);
                cell5.setCellValue(data.getAverage_workdays());//平均工天
                HSSFCell cell6 = row.createCell(6);
                cell6.setCellValue(data.getWorkdays_factor());//工天因数
                HSSFCell cell7 = row.createCell(7);
                cell7.setCellValue(data.getRemove_error_rate());//消灭错漏率
                HSSFCell cell8 = row.createCell(8);
                cell8.setCellValue(data.getStandard_score());//标准成绩
                 HSSFCell cell9 = row.createCell(9);
                cell9.setCellValue(data.getPerformance_evaluation());//质量评定
            }
            String fileName = "个人质量评估列表导出";
